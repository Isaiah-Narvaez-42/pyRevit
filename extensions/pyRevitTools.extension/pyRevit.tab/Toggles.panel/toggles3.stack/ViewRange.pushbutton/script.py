...
PLANE_ORDER = [DB.PlanViewPlane.TopClipPlane, DB.PlanViewPlane.CutPlane, 
    ...
    def Execute(self, uiapp):
        try:
            if self.context and self.new_values:
                self.context._update_view_range_internal(self.new_values, self.new_levels)
        except Exception as e:
            if hasattr(self.context, 'view_model'):
                self.context.view_model.warning_message = "Error executing view range update: {}".format(str(e))
...
class Context(object):
    ...
    def __init__(self, view_model):
        ...
        self.length_unit = doc.GetUnits().GetFormatOptions(DB.SpecTypeId.Length).GetUnitTypeId()
        self.height_data = {}
        self.offset_data = {}
        self.level_data = {}
    ...
    def update_view_range(self, new_values, new_levels=None):
        ...
        if self.source_view.IsTemplate:
            self.view_model.warning_message = "Cannot modify view range - this is a view template"
            return False
        ...
        try:
            ...
            with revit.Transaction("Update View Range", doc=revit.doc):
                ...
                for plane, offset_str in new_values.items():
                    ...
                    if offset_str and offset_str.strip() and offset_str != "-" and offset_str.upper() != "N/A":
                        try:
                            offset_display = float(offset_str)
                            offset_internal = DB.UnitUtils.ConvertToInternalUnits(offset_display, self.length_unit)
            ...
            self.view_model.warning_message = "Error updating view range: {}".format(str(e))
            return False
    ...
    def _validate_view_range_order(self, new_values, new_levels=None):
        try:
            ...
            for plane, offset_str in new_values.items():
                ...
                if offset_str and offset_str.strip() and offset_str != "-":
                    try:
                        ...
                        if level:
                            level_elevation = DB.UnitUtils.ConvertFromInternalUnits(level.ProjectElevation, self.length_unit)
                            elevations[plane] = level_elevation + offset_value
                    except ValueError:
                        forms.alert(
                            "Invalid Input Format!\n\nThe value '{}' for {} is not a valid number.\n\n"
                            "Please enter a numeric value (e.g., 4.5, -2.0, 0)".format(offset_str, PLANES[plane][1]),
                            title="Invalid Number Format", warn_icon=True
                        )
                        self.view_model.warning_message = "Invalid number format: {}".format(offset_str)
                        return False
            ...
            checks = [
                (DB.PlanViewPlane.TopClipPlane, DB.PlanViewPlane.CutPlane, "Top Clip Plane", "Cut Plane"),
                (DB.PlanViewPlane.CutPlane, DB.PlanViewPlane.BottomClipPlane, "Cut Plane", "Bottom Clip Plane"),
                (DB.PlanViewPlane.BottomClipPlane, DB.PlanViewPlane.ViewDepthPlane, "Bottom Clip Plane", "View Depth Plane")
            ]
            ...
            for higher_plane, lower_plane, higher_name, lower_name in checks:
                ...
                if higher_elev is not None and lower_elev is not None and higher_elev < lower_elev:
                    forms.alert(
                        ...
                        "3. Bottom Clip Plane\n4. View Depth Plane (lowest offset)".format(
                            higher_name, offset_values.get(higher_plane, 0),
                            lower_name, offset_values.get(lower_plane, 0)
                        ),
                        title="Invalid View Range", warn_icon=True
                    )
                    self.view_model.warning_message = "{} must be >= {}".format(higher_name, lower_name)
                    return False
            
            return True
            
        except Exception as e:
            self.view_model.warning_message = "Error validating view range: {}".format(str(e))
            return False
    ...
    def _populate_available_levels(self):
        ...
        try:
            ...
            level_items.append(LevelItem("Unlimited", DB.ElementId.InvalidElementId, None, True))
            
            # Add actual levels
            for level in levels:
                level_item = LevelItem(level.Name, level.Id, level.ProjectElevation, False)
    ...
    def _set_current_level_selections(self, view_range):
        ...
        try:
            ...
            for plane in PLANES:
                ...
                if plane == DB.PlanViewPlane.TopClipPlane:
                    ...
                    if level_id and level_id != DB.ElementId.InvalidElementId:
                        ...
                        self.view_model.cutplane_level_name = level.Name if level else "Unknown"
                    else:
            ...
            self.view_model.warning_message = "Error setting level selections: {}".format(str(e))
    ...
    def context_changed(self):
        ...
        try:
            ...
            if isinstance(self.source_view, DB.ViewPlan):
                if self.active_view.get_Parameter(DB.BuiltInParameter.VIEWER_MODEL_CLIP_BOX_ACTIVE).AsInteger() == 1:
                ...
                for plane in PLANE_ORDER:
                    ...
                    plane_elevation = plane_level.ProjectElevation + view_range.GetOffset(plane)
                    self.height_data[plane] = round(DB.UnitUtils.ConvertFromInternalUnits(plane_elevation, self.length_unit), 2)
                    
                    offset_value = round(DB.UnitUtils.ConvertFromInternalUnits(view_range.GetOffset(plane), self.length_unit), 2)
                    ...
                    cut_plane_vertices = [DB.XYZ(c.X, c.Y, plane_elevation) for c in corners]
                ...
                for plane in PLANE_ORDER:
                    ...
                    setattr(self.view_model, prefix + "_elevation", str(self.height_data[plane]))
                    setattr(self.view_model, prefix + "_new_value", str(self.offset_data[plane]))
                ...
                for plane_type in [DB.PlanViewPlane.ViewDepthPlane, DB.PlanViewPlane.CutPlane]:
                    ...
                    if plane_type == DB.PlanViewPlane.CutPlane:
                        view_dir_transform = DB.Transform.CreateTranslation(
                            self.source_view.ViewDirection.Negate() * self.source_view.CropBox.Min.Z
                        )
                        cut_plane_vertices = [view_dir_transform.OfPoint(pt) for pt in cut_plane_vertices]
    ...
    def is_valid(self):
        if not can_use_view_as_source(self.source_view):
            self.view_model.message = "Please select a Plan or Section View in the Project Browser!"
            ...
            self.view_model.can_modify_view = False
            return False
        elif (not context.source_view.CropBoxActive and 
              not context.active_view.get_Parameter(DB.BuiltInParameter.VIEWER_MODEL_CLIP_BOX_ACTIVE).AsInteger() == 1):
            self.view_model.message = ("Please activate the \"Section Box\" on the active view,\n"
                                       "or the \"Crop View\" on the selected view!")
            self.view_model.can_modify_view = False
        else:
            can_modify = isinstance(self.source_view, DB.ViewPlan) and not self.source_view.IsTemplate
            self.view_model.can_modify_view = can_modify
            
            if self.source_view.IsTemplate:
                self.view_model.message = "Showing View Range of [{}]\n(View Template - Cannot Modify)".format(self.source_view.Name)
            else:
                self.view_model.message = "Showing View Range of\n[{}]".format(self.source_view.Name)
            return True
...
class MainViewModel(forms.Reactive):
    def __init__(self):
        ...
        for plane, (rgb, name, prefix) in PLANES.items():
            setattr(self, prefix + "_brush", SolidColorBrush(Color.FromRgb(*[Convert.ToByte(i) for i in rgb])))
...
class MainWindow(forms.WPFWindow):
    ...
    def apply_changes_click(self, sender, e):
        try:
            new_values = {plane: getattr(self.DataContext, prefix + "_new_value") 
            ...
            # Top Plane
            if top_id_int == -1:
                new_levels[DB.PlanViewPlane.TopClipPlane] = DB.ElementId.InvalidElementId
            elif top_id_int is not None:
                new_levels[DB.PlanViewPlane.TopClipPlane] = DB.ElementId(top_id_int)
            
            # Bottom Plane
            if bottom_id_int == -1:
                new_levels[DB.PlanViewPlane.BottomClipPlane] = DB.ElementId.InvalidElementId
            elif bottom_id_int is not None:
                new_levels[DB.PlanViewPlane.BottomClipPlane] = DB.ElementId(bottom_id_int)
            
            # View Depth
            if viewdepth_id_int == -1:
                new_levels[DB.PlanViewPlane.ViewDepthPlane] = DB.ElementId.InvalidElementId
            elif viewdepth_id_int is not None:
                new_levels[DB.PlanViewPlane.ViewDepthPlane] = DB.ElementId(viewdepth_id_int)
            
            context.update_view_range(new_values, new_levels)
        except Exception as ex:
            self.DataContext.warning_message = "Error applying changes: {}".format(str(ex))
        ...
        try:
            ...
            if context.original_level_data:
                ...
                for plane in PLANES:
                    ...
                    if plane == DB.PlanViewPlane.TopClipPlane:
                        if original_level_id and original_level_id != DB.ElementId.InvalidElementId:
                            self.DataContext.topplane_level_id = original_level_id.IntegerValue
                        else:
                        ...
                        if original_level_id and original_level_id != DB.ElementId.InvalidElementId:
                            try:
                                level = context.active_view.Document.GetElement(original_level_id)
                                self.DataContext.cutplane_level_name = level.Name if level else "Unknown"
                            except Exception:
                        ...
                        if original_level_id and original_level_id != DB.ElementId.InvalidElementId:
                            self.DataContext.bottomplane_level_id = original_level_id.IntegerValue
                        else:
                            self.DataContext.bottomplane_level_id = -1
                            
                    elif plane == DB.PlanViewPlane.ViewDepthPlane:
                        if original_level_id and original_level_id != DB.ElementId.InvalidElementId:
                            self.DataContext.viewdepth_level_id = original_level_id.IntegerValue
                        else:
            ...
            self.DataContext.warning_message = "Error resetting values: {}".format(str(ex))
...
def doc_changed(sender, args):
    try:
        affected_ids = list(args.GetModifiedElementIds()) + list(args.GetDeletedElementIds())
        if any(view.Id in affected_ids for view in [context.source_view, context.active_view]):
...
def compare_views(view1, view2):
    if not view1 and not view2:
        ...
        return False
    return view1.Document.GetHashCode() == view2.Document.GetHashCode() and view1.Id == view2.Id
...
def corners_from_bb(bbox):
    ...
    corners = [
        ...
        bbox.Min + DB.XYZ.BasisX * (bbox.Max - bbox.Min).X + DB.XYZ.BasisY * (bbox.Max - bbox.Min).Y,
...
def create_edges(vertices, color):
    return [revit.dc3dserver.Edge(vertices[i-1], vertices[i], color) for i in range(len(vertices))]
